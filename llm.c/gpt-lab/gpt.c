...
float* worker_weight;
float* worker_bias;
int worker_b, worker_T = 0, worker_C, worker_OC;
...
void worker(__attribute__((unused)) int tid){
    while (1) {
        P(&start_ticket);
        if (the_end != 0){
            return;
        ...
        float* out_bt = worker_out + worker_b * worker_T * worker_OC + t * worker_OC;
        float* inp_bt = worker_inp + worker_b * worker_T * worker_C + t * worker_C;
        for (int o = 0; o < worker_OC; o++) {
            float val = (worker_bias != NULL) ? worker_bias[o] : 0.0f;
            float* wrow = worker_weight + o*worker_C;
...
int main(int argc, char** argv) {
    ...
    SEM_INIT(&start_ticket, 0);
    SEM_INIT(&done_flag, 0);
