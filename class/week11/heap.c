...
void upper_heapify(int **ptr ,int n ,int i)
{
    int* index=*ptr;
    if(sizeof(index)<=4*n)
    {
        index=(int*)realloc(index,2*sizeof(index));
    }
    index[n]=i;
    int temp=n;
    while(index[temp]<=index[temp>>1])
    {
        exchange(&index[temp],&index[temp>>1]);
        temp=temp>>1;
    }
}
...
int main()
{
    ...
    printf("\n");
    upper_heapify(&ptr,n,4);
    for (int i = 0; i < n+1; i++)printf("%d\t", ptr[i]);
    free(ptr);
