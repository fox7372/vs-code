...
int main()
{
	...
	insert_int(&table,"key1",10);
	insert_float(&table,"key2",3.14);
	insert_char(&table,"key3","hello");
	insert_int(&table,"{a,b}",100);
	for(int i=0;i<10000;i++)
	{
		insert_int(&table,NULL,i);
	}
	printf("%d\n",search_int(table,10,"key1"));
	printf("%f\n",search_float(table,0,"key2"));
	printf("%s\n",search_char(table,0,"key3"));
	printf("%d\n",search_int(table,0,"{a,b}"));
	printf("%d\n",search_int(table,9999,NULL));
	printf("%d\n",table->count);
	return 0;
}
